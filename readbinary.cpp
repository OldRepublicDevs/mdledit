...
void MDL::DecompileModel(bool bMinimal){
    ...
    //if(!bMinimal) ReportMdl << "File header read.\n";
    ...
    Data.MH.nSupermodelReference = ReadInt(&nPos, 11);
    ...
    Data.MH.nPadding = ReadInt(&nPos, 8);
    ...
    if(Data.MH.NameArray.nCount > 0){
        ...
        while(n < Data.MH.NameArray.nCount){
            ...
            MarkDataBorder(MDL_OFFSET + Data.MH.Names[n].nOffset + Data.MH.Names[n].sName.size());
            n++;
        }
    }
    ...
    if(Data.MH.AnimationArray.nCount > 0 && !bMinimal){
        ...
        while(n < Data.MH.AnimationArray.nCount){
            ...
            Data.MH.Animations[n].EventArray.nOffset = ReadInt(&nPos, 6);
            Data.MH.Animations[n].EventArray.nCount = ReadInt(&nPos, 1);
            Data.MH.Animations[n].EventArray.nCount2 = ReadInt(&nPos, 1);
            Data.MH.Animations[n].nPadding2 = ReadInt(&nPos, 8);
            MarkDataBorder(nPos - 1);

            if(Data.MH.Animations[n].EventArray.nCount > 0){
                Data.MH.Animations[n].Events.resize(Data.MH.Animations[n].EventArray.nCount);
                int i = 0;
                nPos = MDL_OFFSET + Data.MH.Animations[n].EventArray.nOffset;
                //std::cout << string_format("Offset to Animation Events is %i\n", Data.MH.Animations[n].EventArray.nOffset);
                while(i < Data.MH.Animations[n].EventArray.nCount){
                    Data.MH.Animations[n].Events[i].fTime = ReadFloat(&nPos, 2);
                    ReadString(Data.MH.Animations[n].Events[i].sName, &nPos, 3, 32);
                    MarkDataBorder(nPos - 1);
                    i++;
                }
            }
            ...
            LinearizeAnimation(Data.MH.Animations[n].RootAnimationNode, Data.MH.Animations[n].ArrayOfNodes, n);
    ...
    for(int n = 0; n < Data.MH.Animations.size(); n++){
        for(int an = 0; an < Data.MH.Animations[n].ArrayOfNodes.size(); an++){
            ...
            bool bFoundNode = false, bFoundParent = false;
            for(int gn = 0; gn < Data.MH.ArrayOfNodes.size(); gn++){
                Node & geom_node = Data.MH.ArrayOfNodes.at(gn);
                if(!bFoundNode && anim_node.Head.nSupernodeNumber == geom_node.Head.nSupernodeNumber){
                ...
                if(!bFoundParent && anim_node.Head.nParentIndex == geom_node.Head.nSupernodeNumber){
                    anim_node.Head.nParentIndex = geom_node.Head.nNodeNumber;
                    bFoundParent = true;
                }
                if(bFoundNode && bFoundParent) break;
            }
        }
...
void MDL::ParseNode(Node * NODE, int * nNodeCounter, Vector vFromRoot, bool bMinimal){
    ...
    NODE->Head.nSupernodeNumber = ReadInt(&nPos, 5, 2);
    NODE->Head.nNodeNumber = ReadInt(&nPos, 5, 2);
    NODE->Head.nPadding1 = ReadInt(&nPos, 8, 2);

    //ReportMdl << "Reading node " << FH->MH.Names.at(NODE->Head.nNodeNumber).sName << ".\n";
    ...
    if(NODE->Head.nType & NODE_HEADER){
        ...
        if(NODE->Head.ControllerArray.nCount > 0 && !bMinimal){
            ...
            while(n < NODE->Head.ControllerArray.nCount){
                ...
                ctrl.nControllerType = ReadInt(&nPosData, 4);
                ctrl.nUnknown2 = ReadInt(&nPosData, 10, 2);
                ctrl.nValueCount = ReadInt(&nPosData, 5, 2);
                ctrl.nTimekeyStart = ReadInt(&nPosData, 5, 2);
                ctrl.nDataStart = ReadInt(&nPosData, 5, 2);

                MarkBytes(nPosData, 1, 7);
                ctrl.nColumnCount = sBuffer[nPosData];
                nPosData += 1;
                MarkBytes(nPosData, 3, 11);
                ctrl.nPadding[0] = sBuffer[nPosData];
                nPosData += 1;
                ctrl.nPadding[1] = sBuffer[nPosData];
                nPosData += 1;
                ctrl.nPadding[2] = sBuffer[nPosData];
                nPosData += 1;
                MarkDataBorder(nPosData - 1);
                ...
                if(ctrl.nControllerType == CONTROLLER_HEADER_ORIENTATION && NODE->nAnimation != -1){
                ...
                if(nCount * ctrl.nValueCount + ctrl.nDataStart > NODE->Head.ControllerData.size())
                    Error("Missing controller data on an animation controller on node '" + FH->MH.Names.at(NODE->Head.nNodeNumber).sName + "' in animation '" + FH->MH.Animations.at(NODE->nAnimation).sName.c_str() + "'.");
        ...
        if(NODE->Head.ChildrenArray.nCount > 0){
            ...
            while(n < NODE->Head.ChildrenArray.nCount){
                ...
                NODE->Head.Children[n].Head.nParentIndex = NODE->nAnimation == -1 ? NODE->Head.nNodeNumber : NODE->Head.nSupernodeNumber;
    ...
    if(NODE->Head.nType & NODE_EMITTER){
        ...
        NODE->Emitter.nPadding1 = ReadInt(&nPos, 11, 1);
        NODE->Emitter.nFlags = ReadInt(&nPos, 4);
        MarkDataBorder(nPos - 1);
    }

    if(NODE->Head.nType & NODE_REFERENCE){
        ReadString(NODE->Reference.sRefModel, &nPos, 3, 32);
        NODE->Reference.nReattachable = ReadInt(&nPos, 4);
        MarkDataBorder(nPos - 1);
    }
    ...
    try{
        ...
        NODE->Mesh.nOffsetToMdxVertex = ReadInt(&nPos, 6);
        NODE->Mesh.nOffsetToMdxNormal = ReadInt(&nPos, 6);
        NODE->Mesh.nOffsetToMdxColor = ReadInt(&nPos, 6);
        NODE->Mesh.nOffsetToMdxUV1 = ReadInt(&nPos, 6);
        NODE->Mesh.nOffsetToMdxUV2 = ReadInt(&nPos, 6);
        NODE->Mesh.nOffsetToMdxUV3 = ReadInt(&nPos, 6);
        NODE->Mesh.nOffsetToMdxUV4 = ReadInt(&nPos, 6);
        NODE->Mesh.nOffsetToMdxTangent1 = ReadInt(&nPos, 6);
        NODE->Mesh.nOffsetToMdxTangent2 = ReadInt(&nPos, 6);
        NODE->Mesh.nOffsetToMdxTangent3 = ReadInt(&nPos, 6);
        NODE->Mesh.nOffsetToMdxTangent4 = ReadInt(&nPos, 6);

        NODE->Mesh.nNumberOfVerts = ReadInt(&nPos, 1, 2);
        NODE->Mesh.nTextureNumber = ReadInt(&nPos, 1, 2);

        NODE->Mesh.nHasLightmap = ReadInt(&nPos, 7, 1);
        NODE->Mesh.nRotateTexture = ReadInt(&nPos, 7, 1);
        NODE->Mesh.nBackgroundGeometry = ReadInt(&nPos, 7, 1);
        NODE->Mesh.nShadow = ReadInt(&nPos, 7, 1);
        NODE->Mesh.nBeaming = ReadInt(&nPos, 7, 1);
        NODE->Mesh.nRender = ReadInt(&nPos, 7, 1);

        if(bK2) NODE->Mesh.nDirtEnabled = ReadInt(&nPos, 7, 1);
        if(bK2) NODE->Mesh.nPadding1 = ReadInt(&nPos, 11, 1);
        ...
        if(bK2) NODE->Mesh.nPadding2 = ReadInt(&nPos, 11, 1);

        NODE->Mesh.nPadding3 = ReadInt(&nPos, 11, 2);

        NODE->Mesh.fTotalArea = ReadFloat(&nPos, 2);
        NODE->Mesh.nPadding = ReadInt(&nPos, 8);
        NODE->Mesh.nOffsetIntoMdx = ReadInt(&nPos, 6);
        if(!bXbox) NODE->Mesh.nOffsetToVertArray = ReadInt(&nPos, 6);
        MarkDataBorder(nPos - 1);
    }
    catch(const std::exception & e){
        throw mdlexception("In " + FH->MH.Names.at(NODE->Head.nNodeNumber).sName + ", reading trimesh header: " + e.what());
    }
        ...
        if(NODE->Mesh.IndexCounterArray.nCount > 0){
            ...
            NODE->Mesh.nVertIndicesCount = ReadInt(&nPosData, 1);
            MarkDataBorder(nPosData - 1);
        }
        ...
        if(NODE->Mesh.MeshInvertedCounterArray.nCount > 0){
            ...
            NODE->Mesh.nMeshInvertedCounter = ReadInt(&nPosData, 4);
            MarkDataBorder(nPosData - 1);
        }
        ...
        if(NODE->Mesh.FaceArray.nCount > 0){
            ...
            while(n < NODE->Mesh.FaceArray.nCount){
                NODE->Mesh.Faces.at(n).nNodeNumber = NODE->Head.nNodeNumber;
                ...
                if(NODE->Mesh.IndexLocationArray.nCount > 0){
                    ...
                    NODE->Mesh.VertIndices[n].at(0) = ReadInt(&nPosData2, 5, 2);
                    NODE->Mesh.VertIndices[n].at(1) = ReadInt(&nPosData2, 5, 2);
                    NODE->Mesh.VertIndices[n].at(2) = ReadInt(&nPosData2, 5, 2);
            ...
            if(bReadSmoothing){
                n = 0;
                while(n < NODE->Mesh.FaceArray.nCount){
                    NODE->Mesh.Faces.at(n).nSmoothingGroup = ReadInt(&nPosData, 4);
                    MarkDataBorder(nPosData - 1);
                    n++;
                }
            }
    ...
    if(NODE->Head.nType & NODE_SKIN){
        ...
        NODE->Skin.nOffsetToMdxWeightValues = ReadInt(&nPos, 6);
        NODE->Skin.nOffsetToMdxBoneIndices = ReadInt(&nPos, 6);
    ...
    if(NODE->Head.nType & NODE_MESH){
        if(NODE->Mesh.nNumberOfVerts > 0){
            //ReportMdl << "Node " << NODE->Head.nNodeNumber << ": \n";
            ...
            while(n < NODE->Mesh.nNumberOfVerts){
                ...
                NODE->Mesh.Vertices[n].MDXData.nNodeNumber = NODE->Head.nNodeNumber;
                ...
                try{
                    ...
                    if(NODE->Mesh.nMdxDataBitmap & MDX_FLAG_NORMAL){
                        ...
                        if(!bXbox){
                            ...
                            NODE->Mesh.Vertices[n].MDXData.vNormal = DecompressVector(Mdx->ReadInt(&nPosData2, 2));
                        }
                    }
                    if(NODE->Mesh.nMdxDataBitmap & MDX_FLAG_COLOR){
                        ...
                        NODE->Mesh.Vertices[n].MDXData.cColor.fR = Mdx->ReadFloat(&nPosData2, 2);
                        NODE->Mesh.Vertices[n].MDXData.cColor.fG = Mdx->ReadFloat(&nPosData2, 2);
                        NODE->Mesh.Vertices[n].MDXData.cColor.fB = Mdx->ReadFloat(&nPosData2, 2);
                    }
                    ...
                    if(NODE->Mesh.nMdxDataBitmap & MDX_FLAG_TANGENT1){
                        ...
                        for(int b = 0; b < 3; b++){
                            if(bXbox){
                                NODE->Mesh.Vertices[n].MDXData.vTangent1[b] = DecompressVector(Mdx->ReadInt(&nPosData2, 2));
                            }
                            else{
                    ...
                    if(NODE->Mesh.nMdxDataBitmap & MDX_FLAG_TANGENT2){
                        ...
                        for(int b = 0; b < 3; b++){
                            if(bXbox){
                                NODE->Mesh.Vertices[n].MDXData.vTangent2[b] = DecompressVector(Mdx->ReadInt(&nPosData2, 2));
                            }
                            else{
                    ...
                    if(NODE->Mesh.nMdxDataBitmap & MDX_FLAG_TANGENT3){
                        ...
                        for(int b = 0; b < 3; b++){
                            if(bXbox){
                                NODE->Mesh.Vertices[n].MDXData.vTangent3[b] = DecompressVector(Mdx->ReadInt(&nPosData2, 2));
                            }
                            else{
                    ...
                    if(NODE->Mesh.nMdxDataBitmap & MDX_FLAG_TANGENT4){
                        ...
                        for(int b = 0; b < 3; b++){
                            if(bXbox){
                                NODE->Mesh.Vertices[n].MDXData.vTangent4[b] = DecompressVector(Mdx->ReadInt(&nPosData2, 2));
                            }
                            else{
                    ...
                    if(NODE->Head.nType & NODE_SKIN){
                        ...
                        if(bXbox){
                            NODE->Mesh.Vertices[n].MDXData.Weights.nWeightIndex[0] = Mdx->ReadInt(&nPosData2, 5, 2);
                            NODE->Mesh.Vertices[n].MDXData.Weights.nWeightIndex[1] = Mdx->ReadInt(&nPosData2, 5, 2);
                            NODE->Mesh.Vertices[n].MDXData.Weights.nWeightIndex[2] = Mdx->ReadInt(&nPosData2, 5, 2);
                            NODE->Mesh.Vertices[n].MDXData.Weights.nWeightIndex[3] = Mdx->ReadInt(&nPosData2, 5, 2);
                        }
                        else{
                            NODE->Mesh.Vertices[n].MDXData.Weights.nWeightIndex[0] = Mdx->ReadFloat(&nPosData2, 2);
                            NODE->Mesh.Vertices[n].MDXData.Weights.nWeightIndex[1] = Mdx->ReadFloat(&nPosData2, 2);
                            NODE->Mesh.Vertices[n].MDXData.Weights.nWeightIndex[2] = Mdx->ReadFloat(&nPosData2, 2);
                            NODE->Mesh.Vertices[n].MDXData.Weights.nWeightIndex[3] = Mdx->ReadFloat(&nPosData2, 2);
                        }
                    }
                }
                catch(const std::exception & e){
                    std::cout << "Offset into MDX for " << FH->MH.Names.at(NODE->Head.nNodeNumber).sName << " is " << NODE->Mesh.nOffsetIntoMdx << "\n";
                    throw mdlexception("Error reading MDX Data for '" + FH->MH.Names.at(NODE->Head.nNodeNumber).sName  + "': " + e.what());
                }
            ...
            if(NODE->Mesh.nMdxDataSize > 0 && Mdx){
                ...
                if(NODE->Mesh.nMdxDataBitmap & MDX_FLAG_NORMAL){
                    ...
                    if(!bXbox){
                        ...
                        NODE->Mesh.MDXData.vNormal = DecompressVector(Mdx->ReadInt(&nPosData2, 8));
                    }
                }
                if(NODE->Mesh.nMdxDataBitmap & MDX_FLAG_COLOR){
                    nPosData2 = NODE->Mesh.nOffsetIntoMdx + n * NODE->Mesh.nMdxDataSize + NODE->Mesh.nOffsetToMdxColor;
                    NODE->Mesh.MDXData.cColor.fR = Mdx->ReadFloat(&nPosData2, 8);
                    NODE->Mesh.MDXData.cColor.fG = Mdx->ReadFloat(&nPosData2, 8);
                    NODE->Mesh.MDXData.cColor.fB = Mdx->ReadFloat(&nPosData2, 8);
                }
                if(NODE->Mesh.nMdxDataBitmap & MDX_FLAG_UV1){
                    nPosData2 = NODE->Mesh.nOffsetIntoMdx + n * NODE->Mesh.nMdxDataSize + NODE->Mesh.nOffsetToMdxUV1;
                ...
                if(NODE->Mesh.nMdxDataBitmap & MDX_FLAG_TANGENT1){
                    ...
                    for(int b = 0; b < 3; b++){
                        if(bXbox){
                            NODE->Mesh.MDXData.vTangent1[b] = DecompressVector(Mdx->ReadInt(&nPosData2, 8));
                        }
                        else{
                ...
                if(NODE->Mesh.nMdxDataBitmap & MDX_FLAG_TANGENT2){
                    ...
                    for(int b = 0; b < 3; b++){
                        if(bXbox){
                            NODE->Mesh.MDXData.vTangent2[b] = DecompressVector(Mdx->ReadInt(&nPosData2, 8));
                        }
                        else{
                ...
                if(NODE->Mesh.nMdxDataBitmap & MDX_FLAG_TANGENT3){
                    ...
                    for(int b = 0; b < 3; b++){
                        if(bXbox){
                            NODE->Mesh.MDXData.vTangent3[b] = DecompressVector(Mdx->ReadInt(&nPosData2, 8));
                        }
                        else{
                ...
                if(NODE->Mesh.nMdxDataBitmap & MDX_FLAG_TANGENT4){
                    ...
                    for(int b = 0; b < 3; b++){
                        if(bXbox){
                            NODE->Mesh.MDXData.vTangent4[b] = DecompressVector(Mdx->ReadInt(&nPosData2, 8));
                        }
                        else{
                ...
                if(NODE->Head.nType & NODE_SKIN){
                    ...
                    nPosData2 = NODE->Mesh.nOffsetIntoMdx + n * NODE->Mesh.nMdxDataSize + NODE->Skin.nOffsetToMdxBoneIndices;
                    if(bXbox){
                        NODE->Mesh.MDXData.Weights.nWeightIndex[0] = Mdx->ReadInt(&nPosData2, 8, 2);
                        NODE->Mesh.MDXData.Weights.nWeightIndex[1] = Mdx->ReadInt(&nPosData2, 8, 2);
                        NODE->Mesh.MDXData.Weights.nWeightIndex[2] = Mdx->ReadInt(&nPosData2, 8, 2);
                        NODE->Mesh.MDXData.Weights.nWeightIndex[3] = Mdx->ReadInt(&nPosData2, 8, 2);
                    }
                    else{
                        NODE->Mesh.MDXData.Weights.nWeightIndex[0] = Mdx->ReadFloat(&nPosData2, 8);
                        NODE->Mesh.MDXData.Weights.nWeightIndex[1] = Mdx->ReadFloat(&nPosData2, 8);
                        NODE->Mesh.MDXData.Weights.nWeightIndex[2] = Mdx->ReadFloat(&nPosData2, 8);
                        NODE->Mesh.MDXData.Weights.nWeightIndex[3] = Mdx->ReadFloat(&nPosData2, 8);
                    }
                }
                Mdx->MarkDataBorder(NODE->Mesh.nOffsetIntoMdx + (n + 1) * NODE->Mesh.nMdxDataSize - 1);
            }
    ...
    if(NODE->Head.nType & NODE_AABB){
        try{
            ...
            throw mdlexception("In " + FH->MH.Names.at(NODE->Head.nNodeNumber).sName + ", reading aabb: " + e.what());
        }
    }

    if(NODE->Head.nType & NODE_SABER){
        NODE->Saber.nOffsetToSaberVerts = ReadInt(&nPos, 6);
        NODE->Saber.nOffsetToSaberUVs = ReadInt(&nPos, 6);
        NODE->Saber.nOffsetToSaberNormals = ReadInt(&nPos, 6);
        ...
        while(n < 176){
            ...
            NODE->Saber.SaberData[n].vUV1.fX = ReadFloat(&nPosData2, 2);
            NODE->Saber.SaberData[n].vUV1.fY = ReadFloat(&nPosData2, 2);
            MarkDataBorder(nPosData2 - 1);

            NODE->Saber.SaberData[n].vNormal.fX = ReadFloat(&nPosData3, 2);
            NODE->Saber.SaberData[n].vNormal.fY = ReadFloat(&nPosData3, 2);
            NODE->Saber.SaberData[n].vNormal.fZ = ReadFloat(&nPosData3, 2);
            MarkDataBorder(nPosData3 - 1);
            n++;
        }
    }
...
void MDL::DetermineSmoothing(){
        ...
        if(Data.MH.ArrayOfNodes.at(n).Head.nType & NODE_MESH && !(Data.MH.ArrayOfNodes.at(n).Head.nType & NODE_SABER)){
            ...
            for(int f = 0; f < Data.MH.ArrayOfNodes.at(n).Mesh.Faces.size(); f++){
                ...
                if(bDebug){
                    ...
                    //file << Data.MH.Names.at(Data.MH.ArrayOfNodes.at(n).Head.nNodeNumber).cName << " > face " << f << " > Bitangent " << face.vBitangent.Print() << ", Tangent " << face.vTangent.Print() << ", Normal " << face.vNormal.Print() << ", Bitangent × Tangent "<<vCross.Print() << "\n";
                }
    ...
    for(int pg = 0; pg < Data.MH.PatchArrayPointers.size(); pg++){
        ...
        //ReportMdl << "Current patch count: " << nPatchCount << "\n";
        ...
        int nFirstNameIndex = Data.MH.PatchArrayPointers.at(pg).front().nNodeNumber;
        ...
        if(nPatchCount<2) bSingleNormal = false;
            ...
            Vector FirstNormal = GetNodeByNameIndex(Data.MH.PatchArrayPointers.at(pg).at(0).nNodeNumber).Mesh.Vertices.at(Data.MH.PatchArrayPointers.at(pg).at(0).nVertex).MDXData.vNormal;
            for(int p = 0; p < nPatchCount && bSingleNormal; p++){
                if(!FirstNormal.Compare(GetNodeByNameIndex(Data.MH.PatchArrayPointers.at(pg).at(p).nNodeNumber).Mesh.Vertices.at(Data.MH.PatchArrayPointers.at(pg).at(p).nVertex).MDXData.vNormal, fDiff)){
                    bSingleNormal = false;
                }
            }
        }
        ...
        for(int nj = 0; nj < nPatchCount; nj++){
            if(Data.MH.PatchArrayPointers.at(pg).at(0).nNodeNumber != Data.MH.PatchArrayPointers.at(pg).at(nj).nNodeNumber) bCombined = true;
        }
        ...
        for(int p = 0; p < nPatchCount; p++){
            ...
            file << "-> Patch " << p << "/" << nPatchCount - 1 << " (" << Data.MH.Names.at(patch.nNodeNumber).sName << ", vert " << patch.nVertex << ", faces";
            ...
            for(int f = 0; f < patch.FaceIndices.size(); f++){
                Face & face = GetNodeByNameIndex(patch.nNodeNumber).Mesh.Faces.at(patch.FaceIndices.at(f));
                Vertex & v1 = GetNodeByNameIndex(patch.nNodeNumber).Mesh.Vertices.at(face.nIndexVertex[0]);
                Vertex & v2 = GetNodeByNameIndex(patch.nNodeNumber).Mesh.Vertices.at(face.nIndexVertex[1]);
                Vertex & v3 = GetNodeByNameIndex(patch.nNodeNumber).Mesh.Vertices.at(face.nIndexVertex[2]);
                Vector Edge1 = v2 - v1;
                Vector Edge2 = v3 - v1;
                Vector Edge3 = v3 - v2;
                ...
                if(bCreaseAngle && Angle(vNormalBase, vAdd) > static_cast<double>(nCreaseAngle)) continue;
            ...
            if(patch.bUniform){
                ...
                for(int op = 0; op < Data.MH.PatchArrayPointers.at(pg).size(); op++){
                    ...
                    if(op != p){
                        for(int f = 0; f < patch2.FaceIndices.size(); f++){
                            ...
                            if(face.fAreaUV <= 0.000001) bBadUV = true;
                            if(face.fArea <= 0.000001) bBadGeo = true;
                ...
                if(vCheck.Compare(vNormal, fDiff)){
                    bFound = true;
            ...
            if(bFound){
                ...
                nBadGeo++;
                ssVN << "Bad geometry for vertex normal in group " << pg << ", patch " << p << "/" << nPatchCount - 1 << " (" << Data.MH.Names.at(patch.nNodeNumber).sName << ", vert " << patch.nVertex << ")\n";
            }
            else{
                file << "\n:(    NO MATCH FOUND!\n";
                nFailedNormals++;
                ssVN << "No match for vertex normal in group " << pg << ", patch " << p << "/" << nPatchCount - 1 << " (" << Data.MH.Names.at(patch.nNodeNumber).sName << ", vert " << patch.nVertex << ")\n";
            }
            //if(nFailedNormals == nPatchCount) ReportMdl << "No matching vertex normals in " << (bCombined?"combined ":"") << "patch group " << pg << "" << (bCombined?"":" (") << (bCombined?"":Data.MH.Names.at(patch.nNodeNumber).sName) << (bCombined?"":")") << ".\n";
            //else if(p+1 == nPatchCount) ReportMdl << ssVN.str();

            //Do tangent space calculation
            if(GetNodeByNameIndex(patch.nNodeNumber).Mesh.nMdxDataBitmap & MDX_FLAG_TANGENT1 && bDebug){
                ...
                if(bFound){
                    if(patch.SmoothedPatches.size() == 0){
                        ...
                        file << "\n   Correct:";
                        bool b1 = false;
                        if(vCheckB.Compare(vBitangent, fDiff)){
                            nNumOfFoundTSB++;
                            file << " bitangent";
                            b1 = true;
                        }
                        if(vCheckT.Compare(vTangent, fDiff)){
                            nNumOfFoundTST++;
                            file << " tangent";
                            b1 = true;
                        }
                        if(vCheckN.Compare(vNormalTS, fDiff)){
                            nNumOfFoundTSN++;
                            file << " normal";
                            b1 = true;
                        }
                        if(!b1) file << " none";
                        if(vCheckB.Compare(vBitangent, fDiff) && vCheckT.Compare(vTangent, fDiff) && vCheckN.Compare(vNormalTS, fDiff)){
                        ...
                        for(int t = 0; t < patch.SmoothedPatches.size(); t++){
                            ...
                            for(int f = 0; f < curpatch.FaceIndices.size(); f++){
                                Face & face = GetNodeByNameIndex(curpatch.nNodeNumber).Mesh.Faces.at(curpatch.FaceIndices.at(f));
                        ...
                        if(vCheckB2.Compare(vBitangent, fDiff) || vCheckT2.Compare(vTangent, fDiff) || vCheckN2.Compare(vNormalTS, fDiff)){
                            file << "\n   Correct:";
                            if(vCheckB2.Compare(vBitangent, fDiff)){
                                nNumOfFoundTSB++;
                                file << " bitangent";
                                nFound = nFound | 1;
                            }
                            if(vCheckT2.Compare(vTangent, fDiff)){
                                nNumOfFoundTST++;
                                file << " tangent";
                                nFound = nFound | 2;
                            }
                            if(vCheckN2.Compare(vNormalTS, fDiff)){
                                nNumOfFoundTSN++;
                                file << " normal";
                                nFound = nFound | 4;
                            }
                    ...
                    if(vCheckB.Compare(vBitangent, fDiff) || vCheckT.Compare(vTangent, fDiff) || vCheckN.Compare(vNormalTS, fDiff)){
                        ...
                        if(!b1) file << " none";
                        if(vCheckB.Compare(vBitangent, fDiff) && vCheckT.Compare(vTangent, fDiff) && vCheckN.Compare(vNormalTS, fDiff)) bFoundTS = true;
                        else if(bBadUV) file << " (bad UV)";
                        else file << " (incomplete)";
                    }
                    else if(nPatchCount < 12){
                        std::vector<int> nDummyPatchVector;
                ...
                for(int nj = 0; nj < nPatchCount; nj++){
                    if(patch.nNodeNumber != Data.MH.PatchArrayPointers.at(pg).at(nj).nNodeNumber) bCombined = true;
                }
                ...
                if(bFoundTS){
                    ...
                    //ReportMdl << "Bad UV in group " << pg << ", patch " << p << "/" << nPatchCount - 1 << " (" << Data.MH.Names.at(patch.nNodeNumber).sName << ", vert " << patch.nVertex << ")\n";
                    file << "\n:/    BAD UV!\n";
                    nBadUV++;
                }
                else{
                    //ReportMdl << "No match for tangent space in group " << pg << ", patch " << p << "/" << nPatchCount - 1 << " (" << Data.MH.Names.at(patch.nNodeNumber).sName << ", vert " << patch.nVertex << ")\n";
                    file << "\n:(    NO MATCH FOUND!\n";
                }
            }
        }
    ...
    ProgressPos(Data.MH.PatchArrayPointers.size());
    ...
    if(Data.MH.nTotalVertCount > 0) ReportMdl << " (" << std::setprecision(4) << fPercentage << "%)";
    ReportMdl << "\n";
    if(nNumOfFoundNormals < Data.MH.nTotalVertCount && nBadGeo > 0){
        ReportMdl << "  Without bad geometry: " << nNumOfFoundNormals << "/" << (Data.MH.nTotalVertCount - nBadGeo);
        if(Data.MH.nTotalVertCount - nBadGeo > 0) ReportMdl << " (" << std::setprecision(4) << fPercentage2 << "%)";
        ReportMdl << "\n";
    }
    ...
    if(!bGoodEnough) Error("The vector recalculations were off by too much to be able to determine the smoothing groups. Try decompiling with different vertex normal calculation settings.");
        ...
        for(int pg = 0; pg < Data.MH.PatchArrayPointers.size(); pg++){
            for(int p = 0; p < Data.MH.PatchArrayPointers.at(pg).size(); p++){
                ...
                for(int f = 0; f < patch.FaceIndices.size(); f++){
                    GetNodeByNameIndex(patch.nNodeNumber).Mesh.Faces.at(patch.FaceIndices.at(f)).nSmoothingGroup =
                        GetNodeByNameIndex(patch.nNodeNumber).Mesh.Faces.at(patch.FaceIndices.at(f)).nSmoothingGroup | nExistingSG;
                }
            }
        }
    }
...
bool MDL::FindNormal(int nCheckFrom, const int & nPatchCount, const int & nCurrentPatch, const int & nCurrentPatchGroup, const Vector & vNormalBase, const Vector & vNormal, std::vector<int> & CurrentlySmoothedPatches, std::stringstream & file){
    ...
    for(int nCount = nPatchCount - 1; nCount >= nCheckFrom; nCount--){
        if(nCount != nCurrentPatch){
            ...
            //ReportMdl << "Working (" << vWorking.fX << ", " << vWorking.fY << ", " << vWorking.fZ << ").\n";

            //Add the current patch
            for(int f = 0; f < ourpatch.FaceIndices.size(); f++){
                Face & face = GetNodeByNameIndex(ourpatch.nNodeNumber).Mesh.Faces.at(ourpatch.FaceIndices.at(f));
                Vertex & v1 = GetNodeByNameIndex(ourpatch.nNodeNumber).Mesh.Vertices.at(face.nIndexVertex[0]);
                Vertex & v2 = GetNodeByNameIndex(ourpatch.nNodeNumber).Mesh.Vertices.at(face.nIndexVertex[1]);
                Vertex & v3 = GetNodeByNameIndex(ourpatch.nNodeNumber).Mesh.Vertices.at(face.nIndexVertex[2]);
                Vector Edge1 = v2 - v1;
                Vector Edge2 = v3 - v1;
                Vector Edge3 = v3 - v2;
                ...
                /// Crease Angle Test
                /// if enabled, and the angle between the current normal and our candidate is greater than the crease angle, skip it
                if(bCreaseAngle && Angle(vWorking, vAdd) > static_cast<double>(nCreaseAngle)) continue;
...
int MDL::FindTangentSpace(int nCheckFrom, const int & nPatchCount, const int & nCurrentPatch, const int & nCurrentPatchGroup,
    ...
    for(int nCount = nPatchCount - 1; nCount >= nCheckFrom; nCount--){
        if(nCount != nCurrentPatch){
            ...
            double fDiff = 0.00001;
            if(bXbox) fDiff = 0.01;
            if(vCheckB.Compare(vBitangent, fDiff) || vCheckT.Compare(vTangent, fDiff) || vCheckN.Compare(vNormal, fDiff)){
                file << "\n   Correct:";
                int nReturn = 0;
                if(vCheckB.Compare(vBitangent, fDiff)){
                    file << " bitangent";
                ...
                if(vCheckN.Compare(vNormal, fDiff)){
                    file << " normal";
                    nReturn = nReturn | 4;
                }
                if(bBadUV) file << " (bad UV)";
